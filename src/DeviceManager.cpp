...
DeviceManager::DeviceManager(bool daemon, const QString &primaryAdapterName)
{
    m_daemonMode = daemon;
    m_selectedAdapterName = primaryAdapterName;
...
bool DeviceManager::enableBluetooth(bool enforceUserPermissionCheck)
{
    ...
    if (!m_bluetoothAdapter)
    {
        ...
        QBluetoothHostInfo selectedAdapter;

        if (!m_selectedAdapterName.isEmpty()) {
            for (const QBluetoothHostInfo &hi : adaptersList) {
                if (hi.name() == m_selectedAdapterName) {
                    selectedAdapter = hi;
                    break;
                }
            }
        ...
        if (!selectedAdapter.isNull()) {
            m_bluetoothAdapter = new QBluetoothLocalDevice(selectedAdapter.address());
        } else if (!adaptersList.isEmpty()) {
            m_bluetoothAdapter = new QBluetoothLocalDevice(adaptersList.first().address());
        } else {
...
QStringList DeviceManager::getAvailableAdapterNames() const
{
    QStringList names;
    QList<QBluetoothHostInfo> adaptersList = QBluetoothLocalDevice::allDevices();
    for (const QBluetoothHostInfo &hi : adaptersList) {
